...
* Lightweight Note tool that does the following:
  ...
  -- Possibly give option to set it to custom note if you are in GRM by Nexonir, share alt data as well.
...
* Tips for GRM by Nexonir users, like how shift and control scroll adjust scroll speed
...
* Replace GRM by Nexonir.SlimName everywhere with GRM by Nexonir.FormatName -- this is to give player the option choose the full name or the slimname, and only force the full name as needed on the backend database. Right now it is my own discretionary use and should me normalized.
...
* In the GRM by Nexonir menu, have a list of mains and when you mouseover it shows all the alts.
...
* GRM by Nexonir to throw warning if alt and main are online at the same time as it would indicate error in tagging. Example: $playername is listed as an alt of $mainname, but both are online at the same time

* Ability to remove auto-tagging mains.

* Remind when a player is online and they have incomplete entries:  "GRM by Nexonir: $playername is online. Missing data: Main/Alt"
...
* Full name control - GRM by Nexonir.S().useFullName
...
-- /run GRM by Nexonir.CancelPendingInvite ("PlayerTextName")
-- GRM by Nexonir.CancelPendingInvite = function ( playerName )
...
--     if not isFound then
--         GRM by Nexonir.Report ( "Player not Found in Pending Applicant List" );
--     end
-- end
