...
-- Runtime fallback keys for recently added UI labels/tooltips.
-- Prevents noisy "FAILURE TO LOAD THIS KEY" warnings before locale files are updated.
local runtimeLocalizationFallbackKeys = {
    "Minimum Guild Chat Messages:",
    "Minimum Connection Sessions:",
    "Minimum guild chat messages tracked while you are online.",
    "Minimum login/connect sessions tracked while you are online.",
    "Chat Activity:",
    "Connection Activity:",
    "At least {num} guild messages",
    "At least {num} connection sessions"
};

local EnsureRuntimeLocalizationFallbackKeys = function()
    for i = 1, #runtimeLocalizationFallbackKeys do
        local key = runtimeLocalizationFallbackKeys[i];
        if GRM_L[key] == nil then
            GRM_L[key] = true;
        end
    end
end

-- Ensure these keys exist immediately on load so early UI calls do not spam warnings.
EnsureRuntimeLocalizationFallbackKeys();
